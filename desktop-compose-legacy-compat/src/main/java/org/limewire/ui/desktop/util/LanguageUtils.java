...
public class LanguageUtils {
    ...
    private static final Logger LOG = Logger.getLogger(LanguageUtils.class.getName());
    ...
    public static Locale[] getLocales(Font font) {
        ...
        if (jar != null) {
            ...
            LOG.warning("Could not find bundle jar to determine locales");
        }
      ...
      /*  if (LimeWireUtils.isTestingVersion()) { 
        ...
        if (font != null && !OSUtils.isMacOSX()) {
            for (Iterator<Locale> it = locales.iterator(); it.hasNext();) {
                ...
                if (font.canDisplayUpTo(locale.getDisplayName(locale)) != -1) {
                    it.remove();
                }
            }
        }
    ...
    static void addLocalesFromJar(List<Locale> locales, File jar) {
        ...
        try {
            ...
            LOG.log(Level.WARNING, "Could not determine locales", e);
        } finally {
            if (zip != null) {
                try {
                    zip.close();
