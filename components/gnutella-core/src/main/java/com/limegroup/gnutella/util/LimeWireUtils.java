...
public final class LimeWireUtils {
	 ...
	 */
	private static String LIMEWIRE_VERSION = "5.6.4.3";
   ...
   public static int getServiceVersionNumber() {
     ...
     * Same as '"WireShare "+getLimeWireVersion'.
	 */
	public static String getVendor() {
		return "WireShare " + LIMEWIRE_VERSION;
	}    
    ...
    public static File getTemporarySettingsDirectory() throws IOException {
        File tempDir = FileUtils.createTempFile("WireShare", "-temp").getAbsoluteFile();
        ...
        for(int i = 0; i < 1000; i++) {
            tempDir = new File(tempDirParent, "WireShare-tempdir-" + i);
            if(!tempDir.exists()) {
                if(tempDir.mkdir()) {
                    if(tempDir.exists() && tempDir.isDirectory()) {
                        return tempDir;
    ...
    public static void clearTemporarySettingsDirectories() {
        File tempDir;
        try {
            tempDir = FileUtils.createTempFile("WireShare", "-temp").getAbsoluteFile();
        ...
        for(int i = 0; i < 1000; i++) {
            File dir = new File(tempDirParent, "WireShare-tempdir-" + i);
    ...
    public static File getRequestedUserSettingsLocation() {
        ...
        // this will return <user-home>/.WireShare
        
        // On OSX, this will return <user-home>/Library/Preferences/WireShare
        
        // On Windows, this first tries to find:
        // a) <user-home>/$LIMEWIRE_PREFS_DIR/WireShare
        // b) <user-home>/$APPDATA/WireShare
        // c) <user-home/.WireShare
        ...
        if (OSUtils.isWindows()) {
            ...
            if (appdata != null && appdata.length() > 0) {
                ...
                File tempSettingsDir = new File(appdata, "WireShare");
            ...
            settingsDir = new File(CommonUtils.getUserHomeDir(), "Library/Preferences/WireShare");
        } 
